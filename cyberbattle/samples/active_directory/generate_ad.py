...
def create_network_from_smb_traffic(
    ...
    n_servers: int,
    n_users: int,
    admin_probability: float = 0.8 # 0.2 by default the probability needed to overcome
) -> nx.DiGraph:
    graph = nx.DiGraph()
    ...
    global num_credentials
    num_credentials = 0

    def default_vulnerabilities() -> m.VulnerabilityLibrary:
        lib = {}
        global num_credentials # visible from vulnerabilities generated
    ...
    def breach_vulnerabilities(lib: m.VulnerabilityLibrary) -> m.VulnerabilityLibrary:
        ...
        num_credentials += len(credrandset)
        num_credentials += len(credrandset) * n_servers
        return lib
    ...
    def dc_vulnerabilities(lib: m.VulnerabilityLibrary) -> m.VulnerabilityLibrary:
        ...
        num_credentials += n_users * n_servers
        num_credentials += n_users * n_clients
        return lib
    ...
    num_useful_credentials = 0 # visible from listening services
    ...
    for i in range(1, n_clients):
        ...
        if random.random() > 1-admin_probability:  # TODO: make this value rarer as network size increases? Kinda wonky considering we only have one exploit path really
            props = ["admin"]
...
def new_random_environment(seed: Any, env_type) -> m.Environment:
