...
class NodeTrackingInformation:
    ...
    discovered_vulnerabilities: Set[int] = dataclasses.field(default_factory=set)
    discovered_absent_vulnerabilities: Set[int] = dataclasses.field(default_factory=set)
    vulnerabilities_used: Set[int] = dataclasses.field(default_factory=set)
    ports_surely_working: Set[str] = dataclasses.field(default_factory=set)
...
class AgentActions:
    ...
    def get_discovered_vulnerabilities(self, node_id: model.NodeID) -> Set[int]:
        return self._discovered_nodes[node_id].discovered_vulnerabilities

    def get_absence_discovered_vulnerabilities(self, node_id: model.NodeID) -> Set[int]:
        return self._discovered_nodes[node_id].discovered_absent_vulnerabilities

    def get_vulnerabilities_used(self, node_id: model.NodeID) -> Set[int]:
        return self._discovered_nodes[node_id].vulnerabilities_used


    def get_ports_surely_working(self, node_id: model.NodeID) -> Set[str]:
        return self._discovered_nodes[node_id].ports_surely_working
    ...
    def __mark_nodevulnerabilities_as_discovered(
        self, node_id: model.NodeID, vulnerabilities: List[VulnerabilityID]
    ):
        all_vulnerabilities = self._environment.identifiers.local_vulnerabilities
        all_vulnerabilities.extend(self._environment.identifiers.remote_vulnerabilities)

        vulnerabilities_indices = []
        for v in vulnerabilities:
            if v in all_vulnerabilities:  # ignore not valid vulnerabilities
                vulnerabilities_indices.append(all_vulnerabilities.index(v))

        if node_id in self._discovered_nodes:
            before_count = len(self._discovered_nodes[node_id].discovered_vulnerabilities)
            self._discovered_nodes[
                node_id
            ].discovered_vulnerabilities = self._discovered_nodes[
                node_id
            ].discovered_vulnerabilities.union(
                vulnerabilities_indices
            )
        else:
            before_count = 0
            self._discovered_nodes[node_id] = NodeTrackingInformation(
                discovered_vulnerabilities=set(vulnerabilities_indices)
            )
            # TODO: Checks if problem with discovered properties

        newly_discovered_vulnerabilities = (
            len(self._discovered_nodes[node_id].discovered_vulnerabilities) - before_count
        )
        return newly_discovered_vulnerabilities

    def __mark_nodeabsentvulnerabilities_as_discovered(
        self, node_id: model.NodeID, vulnerabilities: List[VulnerabilityID]
    ):
        all_vulnerabilities = self._environment.identifiers.local_vulnerabilities
        all_vulnerabilities.extend(self._environment.identifiers.remote_vulnerabilities)
        ...
        if node_id in self._discovered_nodes:
            before_count = len(self._discovered_nodes[node_id].discovered_absent_vulnerabilities)
            self._discovered_nodes[
                node_id
            ].discovered_absent_vulnerabilities = self._discovered_nodes[
                node_id
            ].discovered_absent_vulnerabilities.union(
                vulnerabilities_indices
            )
        else:
            before_count = 0
            self._discovered_nodes[node_id] = NodeTrackingInformation(
                discovered_absent_vulnerabilities=set(vulnerabilities_indices)
            )
            # TODO: Checks if problem with discovered properties

        newly_discovered_absent_vulnerabilities = (
            len(self._discovered_nodes[node_id].discovered_absent_vulnerabilities) - before_count
        )
        return newly_discovered_absent_vulnerabilities

    def __mark_vulnerabilities_as_used(
        self, node_id: model.NodeID, vulnerabilities: List[VulnerabilityID]
    ):
        all_vulnerabilities = self._environment.identifiers.local_vulnerabilities
        all_vulnerabilities.extend(self._environment.identifiers.remote_vulnerabilities)
        ...
        if node_id in self._discovered_nodes:
            self._discovered_nodes[
                node_id
            ].vulnerabilities_used = self._discovered_nodes[
                node_id
            ].vulnerabilities_used.union(
                vulnerabilities_indices
            )
        else:
            self._discovered_nodes[node_id] = NodeTrackingInformation(
                vulnerabilities_used=set(vulnerabilities_indices)
            )
        #print(self._discovered_nodes[node_id].vulnerabilities_used)


    def __mark_ports_as_surely_working(
        self, node_id: model.NodeID, ports: List[str]
    ):
        if node_id in self._discovered_nodes:
            self._discovered_nodes[
                node_id
            ].ports_surely_working = self._discovered_nodes[
                node_id
            ].ports_surely_working.union(
                ports
            )
        else:
            self._discovered_nodes[node_id] = NodeTrackingInformation(
                ports_surely_working=set(ports)
            )
    ...
    def __mark_allnodevulnerabilities_as_discovered(self, node_id: model.NodeID):
        node_info: model.NodeInfo = self._environment.network.nodes[node_id]["data"]
        return self.__mark_nodevulnerabilities_as_discovered(node_id, node_info.vulnerabilities)
    ...
    def __mark_node_as_owned(
        ...
        if not is_currently_owned:
            ...
            self.__mark_allnodevulnerabilities_as_discovered(node_id)
    ...
    def __process_outcome(
        ...
        if node_info.status != model.MachineStatus.Running:
            ...
            if self.verbose:
                print("Penalty (target machine not in running state):", self.penalty["machine_not_running"])
            return False, ActionResult(reward=self.penalty["machine_not_running"], outcome=None)
        ...
        if is_global_vulnerability:
            ...
            if throw_if_vulnerability_not_present:
                ...
                if self.verbose:
                    print("Penalty (suspiciousness):", self.penalty["suspiciousness"])
                return False, ActionResult(reward=self.penalty["suspiciousness"], outcome=None)
        ...
        if not self._check_prerequisites(node_id, vulnerability):
            if self.verbose:
                print("Penalty (exploit failed):", failed_penalty)
        ...
        if isinstance(outcome, model.PrivilegeEscalation):
            if outcome.tag in node_info.properties:
                if self.verbose:
                    print("Penalty (repeat):", self.penalty["repeat"])
                return False, ActionResult(reward=self.penalty["repeat"], outcome=outcome)
            ...
            if not last_owned_at:
                if self.verbose:
                    print("Reward (new owned node for privilege escalation): +=", self.value_coefficient, "*", float(node_info.value))
                reward += self.value_coefficient*float(node_info.value)
            ...
            if not last_owned_at:
                if self.verbose:
                    print("Reward (new owned node for lateral move): +=", self.value_coefficient, "*", float(node_info.value))
                reward += self.value_coefficient*float(node_info.value)
        ...
        if already_executed:
            ...
            if (
                ...
                if self.verbose:
                    print("Penalty (repeat):", self.penalty["repeat"])
                reward += self.penalty["repeat"]
        else:
            if self.verbose:
                print("Reward (first time action succeeded): +=", self.first_success_attack_coefficient)
            self.__mark_vulnerabilities_as_used(node_id, [vulnerability_id])
            reward += self.first_success_attack_coefficient
        ...
        reward += self.node_discovered_coefficient * newly_discovered_nodes
        if self.verbose:
            print("Reward (nodes discovered): +=", self.node_discovered_coefficient, "*", newly_discovered_nodes)
        reward += self.credential_discovered_coefficient * newly_discovered_credentials
        if self.verbose:
            print("Reward (credentials discovered): +=", self.credential_discovered_coefficient, "*", newly_discovered_credentials)
        reward -= self.cost_coefficient * vulnerability.cost
        if self.verbose:
            print("Penalty (cost): -=", self.cost_coefficient, "*", vulnerability.cost)
    ...
    def exploit_remote_vulnerability(
        ...
        #print("Source node", source_node_info)
        target_node_info: model.NodeInfo = self._environment.get_node(target_node_id)
        #print("Target node", target_node_info)
        if not source_node_info.agent_installed:
            if self._throws_on_invalid_actions:
                print("Source node not owned")
                raise ValueError(
                    "Agent does not owned the source node '" + node_id + "'"
                )
            else:
                if self.verbose:
                    print("Penalty (invalid action):", self.penalty["invalid_action"])
                return ActionResult(reward=self.penalty["invalid_action"], outcome=None)

        if target_node_id not in self._discovered_nodes:
            if self._throws_on_invalid_actions:
                print("Target node not discovered")
                raise ValueError(
        ...
        succeeded, result = self.__process_outcome(
            ...
            failed_penalty=self.penalty["failed_remote_exploit"],
    ...
    def exploit_local_vulnerability(
        ...
        It returns either a vulnerability outcome object or None
        """
        ...
        succeeded, result = self.__process_outcome(
            ...
            failed_penalty=self.penalty["local_exploit_failed"],
            throw_if_vulnerability_not_present=False,
        )

        if succeeded:
            #print("Succeeded local vulnerability")
            self.__mark_nodevulnerabilities_as_discovered(node_id, [vulnerability_id])
        else:
            #print("Local vulnerability did not work!")
            self.__mark_nodeabsentvulnerabilities_as_discovered(node_id, [vulnerability_id])
    ...
    def connect_to_remote_machine(
        ...
        #print(f"Connect from node {source_node_id} to {target_node_id} on port {port_name} with credential {credential}")
        """
        ...
        if credential not in self._gathered_credentials:
            #print("Credential not gathered")
            #print("Credential not gathered")
            if self._throws_on_invalid_actions:
                raise ValueError(f"Agent has not discovered credential '{credential}'")
            else:
                if self.verbose:
                    print("Penalty (credentials not gathered):", self.penalty["invalid_action"])
                return ActionResult(reward=self.penalty["invalid_action"], outcome=None)

        if not self.__is_passing_firewall_rules(
            source_node.firewall.outgoing, port_name
        ):
            #print("Source node firewall blocking outgoing traffic")
            logger.info(
                f"BLOCKED TRAFFIC: source node '{source_node_id}'"
                + f" is blocking outgoing traffic on port '{port_name}'"
            )
            if self.verbose:
                print("Penalty (blocked outgoing traffic):", self.penalty["blocked_by_local_firewall"])
            return ActionResult(reward=self.penalty["blocked_by_local_firewall"], outcome=None)

        if not self.__is_passing_firewall_rules(
            target_node.firewall.incoming, port_name
        ):
            #print("Target node firewall blocking incoming traffic""")
            logger.info(
                f"BLOCKED TRAFFIC: target node '{target_node_id}'"
                + f" is blocking outgoing traffic on port '{port_name}'"
            )
            if self.verbose:
                print("Penalty (blocked incoming traffic):", self.penalty["blocked_by_remote_firewall"])
            return ActionResult(reward=self.penalty["blocked_by_remote_firewall"], outcome=None)

        target_node_is_listening = port_name in [i.name for i in target_node.services]
        if not target_node_is_listening:
            #print("Target node not listening on port")
            logger.info(
                f"target node '{target_node_id}' not listening on port '{port_name}'"
            )
            if self.verbose:
                print("Penalty (scanning unopen port):", self.penalty["scanning_unopen_port"])
            return ActionResult(reward=self.penalty["scanning_unopen_port"], outcome=None)
        else:
            target_node_data: model.NodeInfo = self._environment.get_node(
                target_node_id
            )
            #print("Target node data", target_node_data)
            if target_node_data.status != model.MachineStatus.Running:
                logger.info("target machine not in running state")
                #print("Target node not running")
                if self.verbose:
                    print("Penalty (target node not running):", self.penalty["machine_not_running"])
                return ActionResult(reward=self.penalty["machine_not_running"], outcome=None)
            ...
            if not self._check_service_running_and_authorized(
                ...
                if self.verbose:
                    print("Penalty (wrong password):", self.penalty["wrong_password"])
                return ActionResult(reward=self.penalty["wrong_password"], outcome=None)
            ...
            if is_already_owned:
                #print("Target node already owned")
                if self.verbose:
                    print("Penalty (target node already owned - repeat):", self.penalty["repeat"])
                return ActionResult(reward=self.penalty["repeat"], outcome=model.LateralMove())
            ...
            self.__mark_ports_as_surely_working(target_node_id, [port_name])
            ...
            if last_owned_at is None and self.verbose:
                print("Reward (lateral move):", float(target_node_data.value))
            return ActionResult(
                reward=self.value_coefficient*float(target_node_data.value) if last_owned_at is None else 0.0,
