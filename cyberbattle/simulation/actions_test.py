...
ENV_IDENTIFIERS = model.Identifiers(
    local_vulnerabilities=['UACME61', 'UACME67', 'MimikatzLogonpasswords', 'UACME61', 'ListNeighbors', 'DumpCreds'],
    remote_vulnerabilities=['RDPBF', 'ScanSharepointParentDirectory'],
...
def actions_on_simple_environment() -> actions.AgentActions:
    ...
    env = model.Environment(network=model.create_network(NODES),
                            ...
                            lastModified=datetime.utcnow())
    print(env)
    print(actions.AgentActions(env))
    return actions.AgentActions(env)
...
def test_list_vulnerabilities_function(actions_on_single_node_environment: Fixture,
    ...
    print(actions_on_single_node_environment.list_vulnerabilities_in_target('a'))
